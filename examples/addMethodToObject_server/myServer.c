...
The method callback takes as input a custom data pointer attached to the method node, the identifier of the object from which the method is called, and two arrays for the input and output arguments. The input and output arguments are all of type Variant. Each variatn may in turn contain a (multi-dimensional) array or scalar of any data type.
...
The method takes a string scalar and returns a string scalar with the "Hello" prepended. The type and length of the input arguments is check internally by the SDK, so that we don't have to verify the arguments in the callback. */

static UA_StatusCode
helloWorldMethodCallback(UA_Server *server,
			const UA_NodeId *sessionId, void *sessionHandler,
...
addHelloWorldMethod(UA_Server *server) {
	...
	UA_Server_addMethodNode(server, UA_NODEID_NUMERIC(1, 62541),
				...
				helloAttr, &helloWorldMethodCallback, 1, &inputArgument, 1, &outputArgument, NULL, NULL);
}

/* Increase Array Values Method

The method takes an array of 5 integers and a scalar as inputs. It returns a copy of the array with every entry increased by the scalar. */
...
addIncInt32ArrayMethod(UA_Server *server) {
	...
	UA_Server_addMethodNode(server, UA_NODEID_STRING(1, "IncInt32ArrayValues"),
				...
				incAttr, &IncInt32ArrayMethodCallback, 2, inputArguments, 1, &outputArgument, NULL, NULL);
}
