...
// When a value changes continuously i.e. system time, updating the value in a tight loop
// would take up a lot of resources. Value callbacks allow to synchronizze a variable
// value with an external representation. They attach callbacks to the variable that
// are executed before every read and after every write operation.
...
// With value callbacks, the value is still stored in the variable node. So-called data
// sources go one step further. The server redirects every read and write request
// to a callback function. Upon reading, the callback provides a copy of the current
// value. Internally, the data source needs to implement its own memory management.
